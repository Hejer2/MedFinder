...
export default function DoctorSettings() {
  ...
  return (
    <div className="min-h-screen bg-slate-55 pt-8 pb-20">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">Settings</h1>
          <p className="text-slate-550 mt-1">Manage your doctor account settings and preferences.</p>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="lg:w-1/4 flex-shrink-0">
            <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden sticky top-24">
              {/* Profile Brief */}
              <div className="p-6 border-b border-slate-105 flex items-center gap-4 bg-gradient-to-br from-white to-slate-50">
                <img src={avatarUrl} alt="Avatar" className="w-16 h-16 rounded-2xl object-cover shadow-sm" />
                <div className="overflow-hidden">
                  <div className="font-bold text-slate-900 truncate">{user?.name || 'Doctor'}</div>
                  <div className="text-xs text-primary-600 font-semibold bg-primary-50 px-2 py-0.5 rounded-md inline-block mt-1">Doctor Account</div>
                </div>
              </div>
                ...
                >
                  ...
                  {activeTab === 'notifications' && (
                    <div>
                      ...
                      <div className="space-y-4 max-w-2xl">
                        <ToggleRow label="Email Notifications" desc="Receive email for new appointment requests" checked={notifications.email} onChange={(val) => handleNotificationChange('email', val)} />
                        <ToggleRow label="SMS Reminders" desc="Get text reminders before appointments" checked={notifications.sms} onChange={(val) => handleNotificationChange('sms', val)} />
                        <ToggleRow label="Review Alerts" desc="Notify when patients leave reviews" checked={notifications.reviews} onChange={(val) => handleNotificationChange('reviews', val)} />
                      </div>
                    </div>
                  )}
                    ...
                    <form onSubmit={handlePasswordSubmit}>
                      ...
                      <div className="space-y-6 max-w-2xl">
                        <FormRow label="Current Password">
                          <input type="password" required value={oldPassword} onChange={e => setOldPassword(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 outline-none transition-all" />
                        </FormRow>
                        <FormRow label="New Password">
                          <input type="password" required value={newPassword} onChange={e => setNewPassword(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 outline-none transition-all" />
                        </FormRow>
                        <FormRow label="Confirm New Password">
                          <input type="password" required value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 outline-none transition-all" />
                        </FormRow>
                        <div className="pt-4 flex justify-end">
                          <button type="submit" disabled={passwordMutation.isPending} className="px-8 py-3 bg-primary-600 hover:bg-primary-700 text-white font-bold rounded-xl transition-all shadow-md shadow-primary-500/20">
                  ...
                  {activeTab === 'help' && (
                    <div className="space-y-6">
                      ...
                      <p className="text-sm text-slate-600 leading-relaxed font-medium">Have questions or need assistance with your MedFinder practice account? Our support team is here to help you configure your profile, manage bookings, and troubleshoot scheduling issues.</p>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4">
                        <div className="p-5 border border-slate-100 rounded-2xl space-y-2">
                          ...
                          <p className="text-xs font-bold text-primary-600">support@medfinder.com</p>
                        </div>
                        <div className="p-5 border border-slate-100 rounded-2xl space-y-2">
                          <h3 className="font-extrabold text-sm text-slate-800">Hotline Help</h3>
                          <p className="text-xs font-bold text-slate-605">+216 71 999 888</p>
                        </div>
                      </div>
                    </div>
                  )}
...
// Subcomponents helper to prevent code cluttering
  ...
  return (
    <div className="flex items-center justify-between p-4 border border-slate-100 rounded-2xl bg-slate-50/30">
      ...
      <button 
        ...
        className={`w-11 h-6 rounded-full transition-colors relative ${checked ? 'bg-primary-600' : 'bg-slate-200'}`}
      >
